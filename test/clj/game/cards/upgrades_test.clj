...
(deftest caprice-nisei
  ...
  (testing "Innermost ICE is moved while passing"
    (do-game
      (new-game {:corp {:hand ["Caprice Nisei" "Thimblerig" "Vanilla"]}
                 :runner {:hand ["Chisel"]}})
      (play-from-hand state :corp "Caprice Nisei" "New remote")
      (play-from-hand state :corp "Thimblerig" "Server 1")
      (play-from-hand state :corp "Vanilla" "HQ")
      (take-credits state :corp)
      (let [caprice (get-content state :remote1 0)
            quandary (get-ice state :remote1 0)
        ...
        (is (not (empty? (:prompt (get-corp)))) 
            "Caprice  triggers after passing and swapping innermost ICE")
        (run-jack-out state)
        ...
        (run-continue state)
        (is (not (empty? (:prompt (get-corp)))) 
            "Caprice triggers if innermost ICE is trashed on encounter")))))
